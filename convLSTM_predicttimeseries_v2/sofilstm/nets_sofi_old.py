...
def sofi_decoder(x, y, keep_prob, phase, img_channels, truth_channels, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    with tf.compat.v1.variable_scope('convlstm_layer'):
        ### ---- LSTM STUFF
    ...
    tf.compat.v1.summary.image('input_image_mean', get_image_summary(tf.expand_dims(tf.math.reduce_mean(input_tensor=x,axis=-1),0)))
    tf.compat.v1.summary.image('input_image_std', get_image_summary(tf.expand_dims(tf.math.reduce_mean(input_tensor=x,axis=-1),0)))
