...
# X = tf.placeholder("float32", shape=(None, None, None, 1), name="X")# tf.Variable(x, name="X")

# I = X
# r=2
# bsize, a, b, c = I.get_shape().as_list()
# bsize = tf.shape(I)[0] # Handling Dimension(None) type for undefined batch dim
# X = tf.reshape(I, (tf.shape(I)[0], tf.shape(I)[1], tf.shape(I)[2], r, r))
# X = tf.transpose(X, (0, 1, 2, 4, 3))  # bsize, a, b, 1, 1
# X = tf.split(1, 64, X)  # a, [bsize, b, r, r]
# X = tf.concat(2, [tf.squeeze(x, axis=1) for x in X])  # bsize, b, a*r, r
# X = tf.split(1, b, X)  # b, [bsize, a*r, r]
# X = tf.concat(2, [tf.squeeze(x, axis=1) for x in X])  # bsize, a*r, b*r
