...
def deconv2d_bn_relu_res(x, w_size, num_outputs, stride, keep_prob_, phase, scope):
    ...
    resized_input = tf.image.resize(x, [h * 2, w * 2], method=tf.image.ResizeMethod.BILINEAR)
    conv_2d = tf.compat.v1.layers.separable_conv2d(resized_input, num_outputs,
                                        kernel_size=w_size,
                                        strides=stride,
                                        padding="same",
...
def convlstm(inputs, name = "conv_lstm_cell", Nx=8, Ny=8, nchannels=1024, n_hiddens = 32, kernel_shape=[3, 3]):
    ...
    mysize = tf.shape(input=inputs) 
    batch_size = mysize[0]
    initial_state = convlstm_layer.zero_state(batch_size, dtype=tf.float32)
    outputs,_=tf.compat.v1.nn.dynamic_rnn(convlstm_layer,inputs,initial_state=initial_state,time_major=False,dtype="float32")
    return outputs
