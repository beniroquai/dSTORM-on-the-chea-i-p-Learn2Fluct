# -*- coding: utf-8 -*-
"""
Created on Mon Apr  6 09:08:28 2020
...
def conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False):
  with tf.compat.v1.variable_scope('{0}_conv'.format(idx)) as scope:
    ...
    biases = _variable_on_cpu('biases',[num_features],tf.compat.v1.constant_initializer(0.01))

    conv = tf.nn.conv2d(input=inputs, filters=weights, strides=[1, stride, stride, 1], padding='SAME')
...
def transpose_conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False):
  with tf.compat.v1.variable_scope('{0}_trans_conv'.format(idx)) as scope:
    ...
    output_shape = tf.stack([tf.shape(input=inputs)[0], tf.shape(input=inputs)[1]*stride, tf.shape(input=inputs)[2]*stride, num_features]) 
...
def fc_layer(inputs, hiddens, idx, flat = False, linear = False):
  with tf.compat.v1.variable_scope('{0}_fc'.format(idx)) as scope:
    ...
    biases = _variable_on_cpu('biases', [hiddens], tf.compat.v1.constant_initializer(FLAGS.weight_init))
    if linear:
