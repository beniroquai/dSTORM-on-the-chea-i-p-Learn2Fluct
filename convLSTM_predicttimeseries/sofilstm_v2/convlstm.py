...
class BasicConvLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or "basic_convlstm_cell", reuse=self._reuse):
      ...
      kernel = tf.compat.v1.get_variable('kernel', shape=self._kernel_size+[inp_channel, out_channel])
      concat = tf.nn.conv2d(input=concat, filters=kernel, strides=(1,1,1,1), padding='SAME')
...
if __name__ == '__main__':
    if (0):
        ...
        size_kernel = 3
        num_kernel = 6
        inputs=tf.compat.v1.placeholder(tf.float32, [ntimesteps,batchsize,Nx,Ny,nchanels])
        # we suppose inputs to be [time, batch_size, row, col, channel]
        ...
        outputs, state = tf.compat.v1.nn.dynamic_rnn(cell, inputs, dtype=inputs.dtype, time_major=True)
