...
class Unet_bn(object):
    ...
    def __init__(self, batchsize=4, Nx=128, Ny=128, img_channels=1, truth_channels=1, ntimesteps=9, cost="mean_squared_error", cost_kwargs={}, **kwargs):
        ...
        self.y = tf.placeholder("float", shape=[batchsize, Nx*2, Ny*2, truth_channels])
        ...
        self.recons = sofi_decoder(self.x, self.y, self.keep_prob, self.phase, self.img_channels, self.truth_channels)
    ...
    def _get_cost(self, cost_name, cost_kwargs):
        ...
        total_pixels = self.nx * self.ny * self.truth_channels
        total_pixels_truth = self.nx * self.ny * self.truth_channels
        
        flat_recons = tf.reshape(self.recons, [-1, total_pixels])
        flat_truths = tf.reshape(self.y, [-1, total_pixels_truth])
