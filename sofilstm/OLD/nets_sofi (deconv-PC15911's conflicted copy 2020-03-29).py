...
def sofi_decoder(x, y, keep_prob, phase, img_channels, truth_channels, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    with tf.variable_scope('convlstm_layer'):
        ...
        with tf.variable_scope("conv2d_1by1"):
            output_end = conv2d(in_node, 1, truth_channels, keep_prob, 'conv2truth_channels')
            #output_end = conv2d_bn_relu(x_image, 1, truth_channels, keep_prob, phase, 'conv2truth_channels')
            #output_end = conv2d_sigmoid(in_node, 1, truth_channels, keep_prob, 'conv2truth_channels')
            #output_end = deconv2d_bn_relu_res(in_node, 1, truth_channels, 1, keep_prob, phase, 'conv2truth_channels')
    ...
    tf.summary.image('input_image_mean', get_image_summary(tf.expand_dims(tf.math.reduce_mean(x,axis=0),0)))
    tf.summary.image('input_image_std', get_image_summary(tf.expand_dims(tf.math.reduce_mean(x,axis=0),0)))
